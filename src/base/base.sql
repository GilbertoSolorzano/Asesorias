...
CREATE TABLE AsesorMateria (
    ...
    CONSTRAINT fk_asesormateria_asesor FOREIGN KEY (matriculaAsesor) REFERENCES Asesor(matricula),
    CONSTRAINT fk_asesormateria_materia FOREIGN KEY (idMateria) REFERENCES Materia(idMateria)
) ENGINE = InnoDB;
...
CREATE TABLE Tema (
    ...
    descripcion TEXT,
    CONSTRAINT fk_tema_materia FOREIGN KEY (idMateria) REFERENCES Materia(idMateria) ON DELETE CASCADE
) ENGINE = InnoDB;
--------------------------------------------------
...
CREATE TABLE Asesoria (
    ...
    CONSTRAINT fk_asesoria_alumno FOREIGN KEY (matriculaAlumno) REFERENCES Alumno(matricula),
    CONSTRAINT fk_asesoria_asesor FOREIGN KEY (matriculaAsesor) REFERENCES Asesor(matricula),
    CONSTRAINT fk_asesoria_tema FOREIGN KEY (idTema) REFERENCES Tema(idTema)
) ENGINE = InnoDB;
-- Tabla Mensaje: mensajes enviados dentro de una asesoría.
CREATE TABLE Mensaje (
    idMensaje INT AUTO_INCREMENT PRIMARY KEY,
    idAsesoria INT NOT NULL,
    matriculaRemitente VARCHAR(50) NOT NULL,
    -- almacena la matrícula del remitente (alumno o asesor)
    mensaje TEXT NOT NULL,
    horaMensaje DATETIME NOT NULL,
    CONSTRAINT fk_mensaje_asesoria FOREIGN KEY (idAsesoria) REFERENCES Asesoria(idAsesoria)
) ENGINE = InnoDB;
--------------------------------------------------
...
CREATE TABLE RespuestaEncuesta (
    ...
    matriculaRespondente VARCHAR(50) NOT NULL,
    -- será la matrícula del alumno o del asesor, según corresponda
    ...
    CONSTRAINT fk_respuesta_pregunta FOREIGN KEY (idPregunta) REFERENCES PreguntaEncuesta(idPregunta),
    CONSTRAINT chk_respuesta_rango CHECK (
        respuesta BETWEEN 1 AND 10
    )
) ENGINE = InnoDB;
--------------------------------------------------
-- Tabla MaterialApoyo
...
CREATE TABLE MaterialApoyo (
    ...
    -- puede ser ruta a un PDF o URL de un video
    descripcion TEXT,
    idMateria INT NOT NULL,
    CONSTRAINT fk_materialapoyo_materia FOREIGN KEY (idMateria) REFERENCES Materia(idMateria)
) ENGINE = InnoDB;
--tabla para el toker de reset password
